...
describe("agent runtime profiles", () => {
  it("ships main, compression, and rescue profiles", () => {
    ...
    expect(runtime.activeProfileId).toBe("3090-ti-ornith-35b-125k-stable");
    expect(runtime.profiles[0]?.modelPath).toContain("ornith-1.0-35b-Q4_K_M.gguf");
    expect(runtime.profiles[1]?.modelPath).toContain("Qwen3.6-27B-Q4_K_M.gguf");
    expect(runtime.profiles[2]?.modelPath).toContain("ornith-1.0-9b-Q8_0.gguf");
