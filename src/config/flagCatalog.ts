...
export const SERVER_FLAGS: FlagDef[] = [
  ...
  { id: "alias", flag: "--alias", label: "Model Alias", section: "Server", type: "string", default: "Ornith1", help: "Name shown to clients for this model in the OpenAI-compatible API.", alwaysEmit: true },
  ...
  // ---- Context & Batching ----
  { id: "ctx-size", flag: "--ctx-size", label: "Context Size", section: "Context & Batching", type: "number", default: 98304, min: 64, max: 1048576, step: 1024, help: "Total context window in tokens shared across all parallel slots. Optimized default is 98304 for the always-on Ornith 35B profile on this RTX 3090 Ti.", alwaysEmit: true },
  { id: "batch-size", flag: "--batch-size", label: "Logical Batch Size", section: "Context & Batching", type: "number", default: 1024, min: 1, max: 65536, step: 1, help: "Maximum tokens processed in one prompt batch during evaluation. 1024 leaves more headroom for long-running agent work.", alwaysEmit: true },
  { id: "ubatch-size", flag: "--ubatch-size", label: "Physical Batch Size", section: "Context & Batching", type: "number", default: 256, min: 1, max: 65536, step: 1, help: "Tokens processed in one physical compute step. 256 is the stable 3090 Ti default for the loaded 35B model.", alwaysEmit: true },
  ...
  { id: "cache-type-k", flag: "--cache-type-k", label: "KV Cache Type (K)", section: "Context & Batching", type: "enum", default: "q8_0", options: ["f16", "f32", "q8_0", "q4_0", "q4_1", "q5_0", "q5_1", "iq4_nl"], help: "Quantization of the key cache. q8_0 gives long context with much lower VRAM use than f16.", alwaysEmit: true },
  { id: "cache-type-v", flag: "--cache-type-v", label: "KV Cache Type (V)", section: "Context & Batching", type: "enum", default: "q8_0", options: ["f16", "f32", "q8_0", "q4_0", "q4_1", "q5_0", "q5_1", "iq4_nl"], help: "Quantization of the value cache. q8_0 gives long context with much lower VRAM use than f16.", alwaysEmit: true },
  ...
  // ---- Sampling ----
  { id: "temp", flag: "--temp", label: "Temperature", section: "Sampling", type: "number", default: 0.8, min: 0, max: 2, step: 0.01, help: "Randomness of sampling. Higher is more creative, lower is more focused." },
  { id: "top-k", flag: "--top-k", label: "Top K", section: "Sampling", type: "number", default: 40, min: 0, max: 100000, step: 1, help: "Keep only the K most likely tokens before sampling. 0 disables." },
  { id: "top-p", flag: "--top-p", label: "Top P", section: "Sampling", type: "number", default: 0.95, min: 0, max: 1, step: 0.01, help: "Nucleus sampling: keep tokens whose cumulative probability reaches this value." },
  ...
  { id: "repeat-penalty", flag: "--repeat-penalty", label: "Repeat Penalty", section: "Sampling", type: "number", default: 1.1, min: 0, max: 4, step: 0.01, help: "Penalty applied to tokens that already appeared. 1 disables." },
  ...
  { id: "reasoning", flag: "--reasoning", label: "Reasoning Mode", section: "Prompts & Templates", type: "enum", default: "auto", options: ["auto", "on", "off"], help: "Controls llama.cpp thinking mode. The optimized local coding profiles force this off so OpenAI-compatible clients receive normal assistant content." },
  ...
  { id: "reasoning-budget", flag: "--reasoning-budget", label: "Reasoning Budget", section: "Prompts & Templates", type: "number", default: -1, min: -1, max: 1048576, step: 1, help: "Maximum tokens the model may spend on reasoning. -1 means unlimited." },
